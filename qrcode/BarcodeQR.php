...
final class BarcodeQR {
	...
	public function paypal($itemname, $price, $qty, $item_id){
	  $paypal_url = "https://www.sandbox.paypal.com/ph/cgi-bin/webscr?cmd=_xclick"; 	
	  $business_name =  "nosajxvier-facilitator@gmail.com";	
	  $item_name = $itemname;
	  $amount = $price;
	  $item_number = $item_id;
	  $qty = $qty;
	  ...
	  $lc = "AU";
	  $bn = "PP-BuyNowBF";
	  $url = $paypal_url.'&business='.$business_name.'&item_name='.$item_name.'&amount='.$amount.'&item_number='.$item_number.'&quantity='.$qty.'&no_shipping='.$no_shipping.'&no_note='.$no_note.'&currency_code='.$currency_code.'&lc='.$lc.'&bn='.$bn;
	 $this->url($url);
	 
	 return $this->draw(300, 'qrcode/images/'.$item_id.".png");
